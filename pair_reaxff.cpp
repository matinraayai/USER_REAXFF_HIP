...
PairReaxFFHIP::~PairReaxFFHIP()
{
	...
	if (setup_flag) {
		...
		Finalize(system, control, data, workspace, &cpu_lists, out_control, mpi_data, TRUE);
	}
...
void PairReaxFFHIP::setup( )
{
	...
	if (setup_flag == 0) {
		...
		//printf("Num nbrs %d \n", num_nbrs);
		Make_List(system->total_cap, num_nbrs, TYP_FAR_NEIGHBOR, HALF_LIST,
                  (cpu_lists+FAR_NBRS));
//		if(!Make_List(system->total_cap, num_nbrs, TYP_FAR_NEIGHBOR, HALF_LIST,
//				(cpu_lists+FAR_NBRS)))
...
//		(cpu_lists+FAR_NBRS)->error_ptr=error;
		...
		Hip_Initialize(system, control, data, workspace, gpu_lists, out_control,
				mpi_data);
	...
	else
	{
		...
		Hip_Adjust_End_Index_Before_ReAllocation(oldN, system->N, gpu_lists);

		//printf("Initial setup done. far numbers gpu %d \n", gpu_lists[FAR_NBRS]->num_intrs);

		// check if I need to shrink/extend my data-structs

		Hip_Reallocate_Part1(system, control, data, workspace, gpu_lists, mpi_data);
		Hip_Reallocate_Part2(system, control, data, workspace, gpu_lists, mpi_data);
	}
...
void PairReaxFFHIP::compute(int eflag, int vflag)
{
	...
	Hip_Reset(system, control, data, workspace, gpu_lists);
	...
	// forces
	Hip_Compute_Forces(system, control, data, workspace, gpu_lists, out_control, mpi_data);
	...
	Hip_Copy_Simulation_Data_Device_to_Host( data, (simulation_data *)data->d_simulation_data);
