        ...
        storage * const workspace )
{
    ...
    system->my_atoms = static_cast<reax_atom*>(scalloc( system->total_cap, sizeof(reax_atom),
                                                        __FILE__, __LINE__ ));

    /* space for keeping restriction info, if any */
    if ( control->restrict_bonds )
    {
        workspace->restricted = static_cast<int*>(scalloc( system->local_cap, sizeof(int),
                                                           __FILE__, __LINE__ ));
        workspace->restricted_list = static_cast<int*>(scalloc( system->local_cap * MAX_RESTRICT,
...
{
    system->cm_entries = static_cast<int*>(srealloc( system->cm_entries, sizeof(int) * local_cap,
                                                     __FILE__, __LINE__ ));
    system->max_cm_entries = static_cast<int*>(srealloc( system->max_cm_entries, sizeof(int) * local_cap,
                                                         __FILE__, __LINE__ ));
}
...
{
    system->my_atoms = static_cast<reax_atom*>(srealloc( system->my_atoms, sizeof(reax_atom) * total_cap,
                                                         __FILE__, __LINE__ ));

    /* list management */
    system->far_nbrs = static_cast<int*>(srealloc( system->far_nbrs, sizeof(int) * total_cap,
                                                   __FILE__, __LINE__ ));
    system->max_far_nbrs = static_cast<int*>(srealloc( system->max_far_nbrs, sizeof(int) * total_cap,
                                                       __FILE__, __LINE__ ));

    system->bonds = static_cast<int*>(srealloc( system->bonds, sizeof(int) * total_cap,
                                                __FILE__, __LINE__ ));
    system->max_bonds = static_cast<int*>(srealloc( system->max_bonds, sizeof(int) * total_cap,
                                                    __FILE__, __LINE__ ));

    system->hbonds = static_cast<int*>(srealloc( system->hbonds, sizeof(int) * total_cap,
                                                 __FILE__, __LINE__ ));
    system->max_hbonds = static_cast<int*>(srealloc( system->max_hbonds, sizeof(int) * total_cap,
                                                     __FILE__, __LINE__ ));
}
        ...
        storage * const workspace, int local_cap )
{
    ...
    if ( control->ensemble == nhNVT )
    {
        workspace->v_const = static_cast<rvec*>(smalloc( local_rvec, __FILE__, __LINE__ ));
    }

    /* storage for analysis */
    if ( control->molecular_analysis || control->diffusion_coef )
    {
        workspace->mark = static_cast<int*>(scalloc( local_cap, sizeof(int),
                                                     __FILE__, __LINE__ ));
        workspace->old_mark = static_cast<int*>(scalloc( local_cap, sizeof(int),
                                                         __FILE__, __LINE__ ));
    }
    else
    {
    ...
    if ( control->diffusion_coef )
    {
        workspace->x_old = static_cast<rvec*>(scalloc( local_cap, sizeof(rvec),
                                                       __FILE__, __LINE__ ));
    }
    else
    {
        ...
        storage * const workspace, int total_cap )
{
    ...
    workspace->total_bond_order = static_cast<real*>(smalloc( total_real, __FILE__, __LINE__ ));
    workspace->Deltap = static_cast<real*>(smalloc( total_real, __FILE__, __LINE__ ));
    workspace->Deltap_boc = static_cast<real*>(smalloc( total_real, __FILE__, __LINE__ ));
    workspace->dDeltap_self = static_cast<rvec*>(smalloc( total_rvec, __FILE__, __LINE__ ));
    workspace->Delta = static_cast<real*>(smalloc( total_real, __FILE__, __LINE__ ));
    workspace->Delta_lp = static_cast<real*>(smalloc( total_real, __FILE__, __LINE__ ));
    workspace->Delta_lp_temp = static_cast<real*>(smalloc( total_real, __FILE__, __LINE__ ));
    workspace->dDelta_lp = static_cast<real*>(smalloc( total_real, __FILE__, __LINE__ ));
    workspace->dDelta_lp_temp = static_cast<real*>(smalloc( total_real, __FILE__, __LINE__ ));
    workspace->Delta_e = static_cast<real*>(smalloc( total_real, __FILE__, __LINE__ ));
    workspace->Delta_boc = static_cast<real*>(smalloc( total_real, __FILE__, __LINE__ ));
    workspace->nlp = static_cast<real*>(smalloc( total_real, __FILE__, __LINE__ ));
    workspace->nlp_temp = static_cast<real*>(smalloc( total_real, __FILE__, __LINE__ ));
    workspace->Clp = static_cast<real*>(smalloc( total_real, __FILE__, __LINE__ ));
    workspace->CdDelta = static_cast<real*>(scalloc( total_cap, sizeof(real), __FILE__, __LINE__ ));
    workspace->vlpex = static_cast<real*>(smalloc( total_real, __FILE__, __LINE__ ));
    workspace->bond_mark = static_cast<int*>(scalloc( total_cap, sizeof(int),
                                                      __FILE__, __LINE__ ));
    ...
    workspace->b_s = static_cast<real*>(scalloc( total_cap, sizeof(real),
                                                 __FILE__, __LINE__ ));
    workspace->b_t = static_cast<real*>(scalloc( total_cap, sizeof(real),
                                                 __FILE__, __LINE__ ));
    workspace->b_prc = static_cast<real*>(scalloc( total_cap, sizeof(real),
                                                   __FILE__, __LINE__ ));
    workspace->b_prm = static_cast<real*>(scalloc( total_cap, sizeof(real),
                                                   __FILE__, __LINE__ ));
    workspace->s = static_cast<real*>(scalloc( total_cap, sizeof(real), __FILE__, __LINE__ ));
    workspace->t = static_cast<real*>(scalloc( total_cap, sizeof(real), __FILE__, __LINE__ ));
    workspace->b = static_cast<rvec2*>(scalloc( total_cap, sizeof(rvec2), __FILE__, __LINE__ ));
    workspace->x = static_cast<rvec2*>(scalloc( total_cap, sizeof(rvec2), __FILE__, __LINE__ ));

    if ( control->cm_solver_pre_comp_type == JACOBI_PC )
    {
        workspace->Hdia_inv = static_cast<real*>(scalloc( total_cap, sizeof(real),
                                                          __FILE__, __LINE__ ));
    }
    if ( control->cm_solver_pre_comp_type == ICHOLT_PC
            || control->cm_solver_pre_comp_type == ILUT_PC
            ...
            || control->cm_solver_pre_comp_type == FG_ILUT_PC )
    {
        workspace->droptol = static_cast<real*>(scalloc( total_cap, sizeof(real),
                                                         __FILE__, __LINE__ ));
    }
    ...
    switch ( control->cm_solver_type )
    {
        case GMRES_S:
        case GMRES_H_S:
            workspace->y = static_cast<real*>(scalloc( control->cm_solver_restart + 1, sizeof(real), __FILE__, __LINE__ ));
            workspace->z = static_cast<real*>(scalloc( control->cm_solver_restart + 1, sizeof(real), __FILE__, __LINE__ ));
            workspace->g = static_cast<real*>(scalloc( control->cm_solver_restart + 1, sizeof(real), __FILE__, __LINE__ ));
            workspace->h = static_cast<real**>(scalloc( SQR(control->cm_solver_restart + 1), sizeof(real*), __FILE__, __LINE__ ));
            workspace->hs = static_cast<real*>(scalloc( control->cm_solver_restart + 1, sizeof(real), __FILE__, __LINE__ ));
            workspace->hc = static_cast<real*>(scalloc( control->cm_solver_restart + 1, sizeof(real), __FILE__, __LINE__ ));
            workspace->v = static_cast<real**>(scalloc( SQR(control->cm_solver_restart + 1), sizeof(real*), __FILE__, __LINE__ ));
            break;

        case CG_S:
            workspace->r = static_cast<real*>(scalloc( total_cap, sizeof(real), __FILE__, __LINE__ ));
            workspace->d = static_cast<real*>(scalloc( total_cap, sizeof(real), __FILE__, __LINE__ ));
            workspace->q = static_cast<real*>(scalloc( total_cap, sizeof(real), __FILE__, __LINE__ ));
            workspace->p = static_cast<real*>(scalloc( total_cap, sizeof(real), __FILE__, __LINE__ ));
#if defined(DUAL_SOLVER)
            workspace->r2 = static_cast<rvec2*>(scalloc( total_cap, sizeof(rvec2), __FILE__, __LINE__ ));
            workspace->d2 = static_cast<rvec2*>(scalloc( total_cap, sizeof(rvec2), __FILE__, __LINE__ ));
            workspace->q2 = static_cast<rvec2*>(scalloc( total_cap, sizeof(rvec2), __FILE__, __LINE__ ));
            workspace->p2 = static_cast<rvec2*>(scalloc( total_cap, sizeof(rvec2), __FILE__, __LINE__ ));
#endif
            break;
        ...
        case BiCGStab_S:
            workspace->y = static_cast<real*>(scalloc( total_cap, sizeof(real), __FILE__, __LINE__ ));
            workspace->g = static_cast<real*>(scalloc( total_cap, sizeof(real), __FILE__, __LINE__ ));
            workspace->z = static_cast<real*>(scalloc( total_cap, sizeof(real), __FILE__, __LINE__ ));
            workspace->r = static_cast<real*>(scalloc( total_cap, sizeof(real), __FILE__, __LINE__ ));
            workspace->d = static_cast<real*>(scalloc( total_cap, sizeof(real), __FILE__, __LINE__ ));
            workspace->q = static_cast<real*>(scalloc( total_cap, sizeof(real), __FILE__, __LINE__ ));
            workspace->p = static_cast<real*>(scalloc( total_cap, sizeof(real), __FILE__, __LINE__ ));
            workspace->r_hat = static_cast<real*>(scalloc( total_cap, sizeof(real), __FILE__, __LINE__ ));
            workspace->q_hat = static_cast<real*>(scalloc( total_cap, sizeof(real), __FILE__, __LINE__ ));
#if defined(DUAL_SOLVER)
            workspace->y2 = static_cast<rvec2*>(scalloc( total_cap, sizeof(rvec2), __FILE__, __LINE__ ));
            workspace->g2 = static_cast<rvec2*>(scalloc( total_cap, sizeof(rvec2), __FILE__, __LINE__ ));
            workspace->z2 = static_cast<rvec2*>(scalloc( total_cap, sizeof(rvec2), __FILE__, __LINE__ ));
            workspace->r2 = static_cast<rvec2*>(scalloc( total_cap, sizeof(rvec2), __FILE__, __LINE__ ));
            workspace->d2 = static_cast<rvec2*>(scalloc( total_cap, sizeof(rvec2), __FILE__, __LINE__ ));
            workspace->q2 = static_cast<rvec2*>(scalloc( total_cap, sizeof(rvec2), __FILE__, __LINE__ ));
            workspace->p2 = static_cast<rvec2*>(scalloc( total_cap, sizeof(rvec2), __FILE__, __LINE__ ));
            workspace->r_hat2 = static_cast<rvec2*>(scalloc( total_cap, sizeof(rvec2), __FILE__, __LINE__ ));
            workspace->q_hat2 = static_cast<rvec2*>(scalloc( total_cap, sizeof(rvec2), __FILE__, __LINE__ ));
#endif
            break;
        ...
        case PIPECG_S:
            ...
            workspace->q = static_cast<real*>(scalloc( total_cap, sizeof(real), __FILE__, __LINE__ ));
            workspace->p = static_cast<real*>(scalloc( total_cap, sizeof(real), __FILE__, __LINE__ ));
            workspace->m = static_cast<real*>(scalloc( total_cap, sizeof(real), __FILE__, __LINE__ ));
            workspace->n = static_cast<real*>(scalloc( total_cap, sizeof(real), __FILE__, __LINE__ ));
            workspace->u = static_cast<real*>(scalloc( total_cap, sizeof(real), __FILE__, __LINE__ ));
            workspace->w = static_cast<real*>(scalloc( total_cap, sizeof(real), __FILE__, __LINE__ ));
#if defined(DUAL_SOLVER)
            workspace->z2 = static_cast<rvec2*>(scalloc( total_cap, sizeof(rvec2), __FILE__, __LINE__ ));
            workspace->r2 = static_cast<rvec2*>(scalloc( total_cap, sizeof(rvec2), __FILE__, __LINE__ ));
            workspace->d2 = static_cast<rvec2*>(scalloc( total_cap, sizeof(rvec2), __FILE__, __LINE__ ));
            workspace->q2 = static_cast<rvec2*>(scalloc( total_cap, sizeof(rvec2), __FILE__, __LINE__ ));
            workspace->p2 = static_cast<rvec2*>(scalloc( total_cap, sizeof(rvec2), __FILE__, __LINE__ ));
            workspace->m2 = static_cast<rvec2*>(scalloc( total_cap, sizeof(rvec2), __FILE__, __LINE__ ));
            workspace->n2 = static_cast<rvec2*>(scalloc( total_cap, sizeof(rvec2), __FILE__, __LINE__ ));
            workspace->u2 = static_cast<rvec2*>(scalloc( total_cap, sizeof(rvec2), __FILE__, __LINE__ ));
            workspace->w2 = static_cast<rvec2*>(scalloc( total_cap, sizeof(rvec2), __FILE__, __LINE__ ));
#endif
            break;
    ...
    /* force related storage */
    workspace->f = static_cast<rvec*>(scalloc( total_cap, sizeof(rvec),
                                               __FILE__, __LINE__ ));

#if defined(TEST_FORCES)
    workspace->dDelta = static_cast<rvec*>(smalloc( total_rvec, __FILE__, __LINE__ ));
    workspace->f_ele = static_cast<rvec*>(smalloc( total_rvec, __FILE__, __LINE__ ));
    workspace->f_vdw = static_cast<rvec*>(smalloc( total_rvec, __FILE__, __LINE__ ));
    workspace->f_bo = static_cast<rvec*>(smalloc( total_rvec, __FILE__, __LINE__ ));
    workspace->f_be = static_cast<rvec*>(smalloc( total_rvec, __FILE__, __LINE__ ));
    workspace->f_lp = static_cast<rvec*>(smalloc( total_rvec, __FILE__, __LINE__ ));
    workspace->f_ov = static_cast<rvec*>(smalloc( total_rvec, __FILE__, __LINE__ ));
    workspace->f_un = static_cast<rvec*>(smalloc( total_rvec, __FILE__, __LINE__ ));
    workspace->f_ang = static_cast<rvec*>(smalloc( total_rvec, __FILE__, __LINE__ ));
    workspace->f_coa = static_cast<rvec*>(smalloc( total_rvec, __FILE__, __LINE__ ));
    workspace->f_pen = static_cast<rvec*>(smalloc( total_rvec, __FILE__, __LINE__ ));
    workspace->f_hb = static_cast<rvec*>(smalloc( total_rvec, __FILE__, __LINE__ ));
    workspace->f_tor = static_cast<rvec*>(smalloc( total_rvec, __FILE__, __LINE__ ));
    workspace->f_con = static_cast<rvec*>(smalloc( total_rvec, __FILE__, __LINE__ ));
    workspace->f_tot = static_cast<rvec*>(smalloc( total_rvec, __FILE__, __LINE__ ));

    if ( system->my_rank == MASTER_NODE )
    {
        workspace->rcounts = static_cast<int*>(smalloc( sizeof(int) * system->nprocs,
                                                        __FILE__, __LINE__ ));
        workspace->displs = static_cast<int*>(smalloc( sizeof(int) * system->nprocs,
                                                       __FILE__, __LINE__ ));
        workspace->id_all = static_cast<int*>(smalloc( sizeof(int) * system->bigN,
                                                       __FILE__, __LINE__ ));
        workspace->f_all = static_cast<rvec*>(smalloc( sizeof(rvec) * system->bigN,
                                                       __FILE__, __LINE__ ));
    }
    else
    {
        ...
        int format )
{
    ...
    H->start = static_cast<int*>(smalloc( sizeof(int) * n_max, __FILE__, __LINE__ ));
    H->end = static_cast<int*>(smalloc( sizeof(int) * n_max, __FILE__, __LINE__ ));
    H->j = static_cast<int*>(smalloc( sizeof(int) * m, __FILE__, __LINE__ ));
    H->val = static_cast<real*>(smalloc( sizeof(real) * m, __FILE__, __LINE__ ));
}
...
void Allocate_Grid( reax_system * const system, MPI_Comm comm )
{
    ...
    g->order = static_cast<ivec*>(scalloc( g->total + 1, sizeof(ivec), __FILE__, __LINE__ ));

    /* allocate the gcells for the new grid */
    g->max_nbrs = (2 * g->vlist_span[0] + 1) * (2 * g->vlist_span[1] + 1)
        * (2 * g->vlist_span[2] + 1) + 3;

    g->cells = static_cast<grid_cell*>(scalloc( total, sizeof(grid_cell), __FILE__, __LINE__ ));
    ...
    g->str = static_cast<int*>(scalloc( total, sizeof(int),__FILE__, __LINE__ ));
    g->end = static_cast<int*>(scalloc( total, sizeof(int), __FILE__, __LINE__ ));
    g->cutoff = static_cast<real*>(scalloc( total, sizeof(real), __FILE__, __LINE__ ));
    g->nbrs_x = static_cast<ivec*>(scalloc( total * g->max_nbrs, sizeof(ivec), __FILE__, __LINE__ ));
    g->nbrs_cp = static_cast<rvec*>(scalloc( total * g->max_nbrs, sizeof(rvec), __FILE__, __LINE__ ));
    ...
    g->rel_box = static_cast<ivec*>(scalloc( total, sizeof(ivec), __FILE__, __LINE__ ));
        ...
        for ( j = g->native_str[1]; j < g->native_end[1]; ++j )
        {
            for ( k = g->native_str[2]; k < g->native_end[2]; ++k )
            {
                g->cells[ index_grid_3d(i, j, k, g) ].atoms =
                   static_cast<int*>(scalloc( g->max_atoms, sizeof(int),
                                              __FILE__, __LINE__ ));
            }
        }
    }
            ...
            for ( j = g->native_str[1]; j < g->native_end[1]; j++ )
            {
                for ( k = g->native_str[2]; k < g->native_end[2]; k++ )
                {
                    sfree( g->cells[ index_grid_3d(i, j, k, g) ].atoms,
                            __FILE__, __LINE__ );
                    g->cells[ index_grid_3d(i, j, k, g) ].atoms = static_cast<int*>(scalloc( realloc->gcell_atoms,
                            sizeof(int), __FILE__, __LINE__ ));
                }
            }
        }
