...
HIP_GLOBAL void k_generate_neighbor_lists_full( reax_atom *my_atoms,
        ...
        int n, int N, int *far_nbrs, int *max_far_nbrs, int *realloc_far_nbrs )
{
    ...
    { 
        ...
        if ( //g.str[index_grid_3d(i, j, k, &g)] <= g.str[index_grid_3d(nbrs_x[0], nbrs_x[1], nbrs_x[2], &g)] && 
                Hip_DistSqr_to_Special_Point(g.nbrs_cp[index_grid_nbrs(i, j, k, itr, &g)], atom1->x) <= cutoff )
        {
...
//HIP_GLOBAL void __launch_bounds__ (1024) k_mt_generate_neighbor_lists( reax_atom *my_atoms,
        ...
        simulation_box my_ext_box, grid g, reax_list far_nbr_list, int n, int N )
{
    ...
    { 
        ...
        if ( (g.str[index_grid_3d(i, j, k, &g)] <= g.str[index_grid_3d( nbrs_x[0], nbrs_x[1], nbrs_x[2], &g)] 
                ...
                && Hip_DistSqr_to_Special_Point(g.nbrs_cp[index_grid_nbrs(i, j, k, itr, &g)], atom1->x) <= cutoff_ji) )
        {
...
HIP_GLOBAL void k_estimate_neighbors_full( reax_atom *my_atoms,
