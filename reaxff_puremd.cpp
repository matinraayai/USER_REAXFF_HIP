...
void* allocate_handle() {
  ...
  pmd_handle->system = static_cast<reax_system*>(smalloc( sizeof(reax_system),
                                                          __FILE__, __LINE__ ));
  pmd_handle->control = static_cast<control_params*>(smalloc( sizeof(control_params),
                                                              __FILE__, __LINE__ ));
  pmd_handle->data = static_cast<simulation_data*>(smalloc( sizeof(simulation_data),
                                                            __FILE__, __LINE__ ));
  pmd_handle->workspace = static_cast<storage*>(smalloc( sizeof(storage),
                                                         __FILE__, __LINE__ ));
#if defined(HAVE_HIP)
  pmd_handle->workspace->d_workspace = static_cast<storage*>(smalloc( sizeof(storage),
                                                                      __FILE__, __LINE__ ));
#endif
  pmd_handle->lists = static_cast<reax_list**>(smalloc( sizeof(reax_list *) * LIST_N,
                                                        __FILE__, __LINE__ ));
  for ( i = 0; i < LIST_N; ++i )
  {
    pmd_handle->lists[i] = static_cast<reax_list*>(smalloc( sizeof(reax_list),
                                                            __FILE__, __LINE__ ));
    pmd_handle->lists[i]->allocated = FALSE;
  }
  pmd_handle->out_control = static_cast<output_controls*>(smalloc( sizeof(output_controls),
                                                                   __FILE__, __LINE__ ));
  pmd_handle->mpi_data = static_cast<mpi_datatypes*>(smalloc( sizeof(mpi_datatypes),
                                                              __FILE__, __LINE__ ));
  return static_cast<void*>(pmd_handle);
}
...
void* setup( const char * const geo_file, const char * const ffield_file,
        const char * const control_file )
{
    puremd_handle* pmd_handle = static_cast<puremd_handle*>(allocate_handle());
